...
export async function queryAdminPublicDataExport(env: Env) {
  ...
  for (const row of commentRows) {
    ...
    current.push({
      id: Number(row.id),
      scores: {
        ethics: Number(row.score_ethics || 0),
        academic: Number(row.score_academic || 0),
        wlb: Number(row.score_wlb || 0),
        funding: Number(row.score_funding || 0),
        graduation: Number(row.score_graduation || 0),
        outcome: Number(row.score_outcome || 0),
      },
